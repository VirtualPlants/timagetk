...
PTRIMAGE_FORMAT createAnalyzeFormat() {
  ...
  f->readImageData=&_readAnalyzeData;
...
int testAnalyzeHeader( char *magic, const char *name __attribute__ ((unused)) ) {
   ...
   if(im->openMode != OM_CLOSE) {
	...
	analyzeHeader->dime.dim[4] = 1; 
	 ...
	 return -1;
      ...
      case DT_NONE:
	fprintf (stderr,
		 "_readAnalyzeHeader: warning: unknown or none data type (%d)...\n",
		 analyzeHeader->dime.datatype );
	if ( 0 ) return -1;
	break ;
      case DT_BINARY:
	fprintf (stderr,
		 "_readAnalyzeHeader: error: binary data type (%d) not supported...\n",
		 analyzeHeader->dime.datatype );
	return -1;
	break ;
      ...
      case DT_COMPLEX:
	fprintf (stderr,
		 "_readAnalyzeHeader: error: complex data type (%d) not supported...\n",
		 analyzeHeader->dime.datatype );
	return -1;
	break ;
      case DT_RGB:
	...
	break ;
      case DT_ALL:
	fprintf (stderr,
		 "_readAnalyzeHeader: error: 'all' type (%d) not supported...\n",
		 analyzeHeader->dime.datatype );
	return -1;
	break ;
      default:
...
int _readAnalyzeData(_image *im) {
  char *proc = "_readAnalyzeData";
  char *data = NULL;
  char *tmp = NULL;

  if(_readImageData(im) < 0) {
    fprintf( stderr, "%s: error when reading data\n", proc );
    return( -1 );
  }

  if ( im->vdim == 3 && im->wdim == 1) {
    size_t i, size;
    size_t v, x, y, z;

    data = (char *)im->data;
    size = (size_t)im->wdim * (size_t)im->xdim * (size_t)im->vdim * (size_t)im->ydim * (size_t)im->zdim;
    tmp = (char*)malloc( size );
    if ( tmp == (char*)NULL ) {
      fprintf(stderr, "%s: error: unable to allocate buffer\n", proc);
      return( -1 );
    }
    for ( i=0, z=0; z<im->zdim; z++ )
    for ( y=0; y<im->ydim; y++ )
    for ( x=0; x<im->xdim; x++ )
    for ( v=0; v<im->vdim; v++, i++ ) {
      tmp[i] = data[z*im->vdim*im->ydim*im->xdim + v*im->ydim*im->xdim + y*im->xdim + x];
    }
  ...
  int j;
   ...
   if ( im->wordKind == WK_FIXED && im->sign == SGN_UNSIGNED ) {
	...
	
	{
	  ...
	  size_t i, size = im->xdim * im->ydim * im->zdim * im->vdim;
	  imin = imax = *buf;
	...
	if ( im->vdim == 1 ) {
	  ...
	  if ( imax < 32768 ) {
	    fprintf( stderr, "%s: 'unsigned short int' type is not allowed in Analyze format\n", proc );
	    fprintf( stderr, "\t 'short int' type will be used instead\n" );
     ...
     if( im->wdim == 2 ) {
       ...
       size_t i, size = im->xdim * im->ydim * im->zdim;
       imin = imax = *buf;
       for (i=0; i<size; i++, buf++) {
	 if ( imax < *buf ) imax = *buf;
	 if ( imin > *buf ) imin = *buf;
...
int writeAnalyzeData(const _image *im) {
  size_t lineSize = im->wdim * im->xdim * im->vdim ;
  size_t size = lineSize * (size_t)im->ydim * (size_t)im->zdim;
  size_t nwrt ;
  char *theData = (char *)im->data ;
  ...
  if ( im->vdim == 3 && im->wdim == 1) {
    ...
    tmp = (char*)malloc( size );
    if ( tmp == (char*)NULL ) {
      fprintf(stderr, "writeAnalyzeData: error: unable to allocate buffer\n");
      return( -1 );
    }
    for ( i=0, z=0; z<im->zdim; z++ )
    for ( y=0; y<im->ydim; y++ )
    for ( x=0; x<im->xdim; x++ )
    for ( v=0; v<im->vdim; v++, i++ ) {
      tmp[z*im->vdim*im->ydim*im->xdim + v*im->ydim*im->xdim + y*im->xdim + x] = data[i];
    }
    data = tmp;
    free( tmp );
  }
...
#else
    nwrt = ImageIO_write(im, data, size);
    if (nwrt != size) {
      return( -1 );
    }
#endif    
