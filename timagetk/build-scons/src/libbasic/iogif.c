/*************************************************************************
 * iogif.c - I procedures for GIF
 *
 * $Id: iogif.c,v 1.1 2000/04/26 15:50:25 greg Exp $
 *
 * AUTHOR:
 * 
 * CREATION DATE: 
 * Wed Apr 26 17:26:27 MET DST 2000
 *
 * ADDITIONS, CHANGES
 *
 */
...
static void* GifError();


byte *RawGIF = (byte*)NULL;	/* The heap array to hold it, raw */
byte *Raster = (byte*)NULL;	/* The raster data stream, unblocked */
...
/*****************************/
void *_readGifImage( char *name, int *dimx, int *dimy, int *dimz )
{
  char *proc = "_readGifImage";
  FILE *fopen();
  ...
  *dimx = *dimy = *dimz = 0;
  ...
  if (aspect) {
    ...
    if (_DEBUG_) fprintf(stderr,"GIF89 aspect = %f\n", normaspect);
  }
  ...
  if (HasColormap)
    {
      r = (byte *) malloc(ColorMapSize * sizeof(byte));
      g = (byte *) malloc(ColorMapSize * sizeof(byte));
      b = (byte *) malloc(ColorMapSize * sizeof(byte));
  ...
  while (1) {
    ...
    if (block == EXTENSION) {  /* parse extension blocks */
      ...
      if (_DEBUG_) fprintf(stderr,"GIF extension type 0x%02x\n", fn);
	...
	if (blocksize == 2) {
	  ...
	  if (_DEBUG_) fprintf(stderr,"GIF87 aspect extension: %d:%d = %f\n\n", 
	...
	if (_DEBUG_) fprintf(stderr,"Comment extension:  ");
	/* read (and ignore) data sub-blocks */
	do {
	  j = 0;  sbsize = NEXTBYTE;
	  while (j<sbsize) {
	    ch = NEXTBYTE;  j++;
	    if (_DEBUG_) fprintf(stderr,"%c", ch);
	  }
	} while (sbsize);
	...
	if (_DEBUG_) fprintf(stderr,"Graphic Control extension\n\n");
	...
	if (_DEBUG_) fprintf(stderr,"unknown GIF extension 0x%02x\n\n", fn);
  ...
  *dimy = Height;
  *dimz = 3;
  theBuf =  malloc(Width * Height * 3);
  org = buf = (unsigned char *) theBuf;
...
/*****************************/
static void * GifError(char *st) {
  fprintf(stderr,"readGifImage: error: %s\n",st);


  if (RawGIF != (byte*)NULL) free(RawGIF);
  if (Raster != (byte*)NULL) free(Raster);
  
  if ( fd != (FILE*)NULL )   fclose( fd );
  
  if ( theBuf != (void*)NULL ) free( theBuf );
