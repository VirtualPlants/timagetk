 ...
 *
 * $Id: iopnm.c,v 1.8 2006/04/14 08:38:55 greg Exp $
 *
 * LICENSE:
                   ...
                   int *dimx, int *dimy, int *dimz, int *bytes )
{
  ...
  *bytes = 0;
  ...
  case Psix :
    ...
    if ( fread( buf, sizeof( unsigned char ), sizeOfBuffer, f ) != (size_t)sizeOfBuffer ) {
      if ( _VERBOSE_ ) 
                     ...
                     int x, int y, int z, int b, void *buf )
{
  ...
  if ( fwrite( bufToWrite, sizeof( unsigned char ), sizeOfBuffer, f ) != (size_t)sizeOfBuffer ) {
    if ( _VERBOSE_ ) 
