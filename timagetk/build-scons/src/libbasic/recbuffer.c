...
void setVerboseInRecBuffer( int v )
{
  _verbose_ = v;
  setVerboseInRecLine( v );
}

void incrementVerboseInRecBuffer(  )
{
  _verbose_ ++;
  incrementVerboseInRecLine();
}

void decrementVerboseInRecBuffer(  )
{
  _verbose_ --;
  if ( _verbose_ < 0 ) _verbose_ = 0;
  decrementVerboseInRecLine();
}
                     ...
                     filterType filterType )
{
  ...
  if ( grdBuf != bufferOut ) {
    if ( ConvertBuffer( grdBuf, FLOAT, bufferOut, typeOut,
                   ...
                   filterType filterType )
{
  ...
  for ( z=0; z<bufferDims[2]; z++ ) {
    ...
    if ( typeOut != FLOAT ) {
      ...
      if ( ConvertBuffer( theYY, FLOAT, sliceOut, typeOut, dimxXdimy ) != 1 ) {
        if ( _verbose_ > 0 )
          fprintf( stderr, " %s: unable to convert buffer.\n", proc );
        free( theXX );
        return( EXIT_ON_FAILURE );
                   ...
                   filterType filterType )
{
  ...
  if ( typeOut != FLOAT ) {
    if ( ConvertBuffer( theZZ, FLOAT, bufferOut, typeOut, bufferDims[2]*dimxXdimy ) != 1 ) {
      if ( _verbose_ > 0 ) {
        fprintf( stderr, " %s: unable to convert buffer", proc );
      }
      free( theSL );
      return( EXIT_ON_FAILURE );
    }
                   ...
                   filterType filterType )
{
  ...
  for ( z=0; z<bufferDims[2]; z++ ) {
    ...
    if ( typeOut != FLOAT ) {
      ...
      if ( ConvertBuffer( theXY, FLOAT, sliceOut, typeOut, dimxXdimy ) != 1 ) {
        if ( _verbose_ > 0 )
          fprintf( stderr, " %s: unable to convert buffer.\n", proc );
        free( theXX );
        return( EXIT_ON_FAILURE );
