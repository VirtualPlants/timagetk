...
int Compute_NotOptT26( int neighbors[27] )
{
    ...
    if ( ( ++TESTS_nb ? neighbors[ 4] : neighbors[ 4] ) != 0 ) {
      ...
      if ( label_vertex[0] != min_label ) { EQUIV_nb++; old_label = label_vertex[ 0 ]; label_vertex[old_label] = min_label; for ( i=old_label+1; i<8; i++ ) if ( label_vertex[i] == old_label ) label_vertex[i] = min_label; }
	  ...
	  { a_label = label_vertex[ 0]; b_label = label_vertex[ 1]; label_exists[ 0] = label_exists[ 1] = 1; if ( a_label < b_label ) { EQUIV_nb++; label_vertex[b_label] = a_label; for ( i=b_label+1; i<8; i++ ) if ( label_vertex[i] == b_label ) label_vertex[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_vertex[a_label] = b_label; for ( i=a_label+1; i<8; i++ ) if ( label_vertex[i] == a_label ) label_vertex[i] = b_label; } }
      if ( ( ++TESTS_nb ? neighbors[ 3] : neighbors[ 3] ) != 0 )
	  { a_label = label_vertex[ 0]; b_label = label_vertex[ 2]; label_exists[ 0] = label_exists[ 2] = 1; if ( a_label < b_label ) { EQUIV_nb++; label_vertex[b_label] = a_label; for ( i=b_label+1; i<8; i++ ) if ( label_vertex[i] == b_label ) label_vertex[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_vertex[a_label] = b_label; for ( i=a_label+1; i<8; i++ ) if ( label_vertex[i] == a_label ) label_vertex[i] = b_label; } }
      if ( ( ++TESTS_nb ? neighbors[ 5] : neighbors[ 5] ) != 0 )
	  { a_label = label_vertex[ 1]; b_label = label_vertex[ 3]; label_exists[ 1] = label_exists[ 3] = 1; if ( a_label < b_label ) { EQUIV_nb++; label_vertex[b_label] = a_label; for ( i=b_label+1; i<8; i++ ) if ( label_vertex[i] == b_label ) label_vertex[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_vertex[a_label] = b_label; for ( i=a_label+1; i<8; i++ ) if ( label_vertex[i] == a_label ) label_vertex[i] = b_label; } }
    ...
    if ( ( ++TESTS_nb ? neighbors[ 10] : neighbors[ 10] ) != 0 ) {
	  ...
	  { a_label = label_vertex[ 0]; b_label = label_vertex[ 4]; label_exists[ 0] = label_exists[ 4] = 1; if ( a_label < b_label ) { EQUIV_nb++; label_vertex[b_label] = a_label; for ( i=b_label+1; i<8; i++ ) if ( label_vertex[i] == b_label ) label_vertex[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_vertex[a_label] = b_label; for ( i=a_label+1; i<8; i++ ) if ( label_vertex[i] == a_label ) label_vertex[i] = b_label; } }
      if ( ( ++TESTS_nb ? neighbors[ 11] : neighbors[ 11] ) != 0 )
	  { a_label = label_vertex[ 1]; b_label = label_vertex[ 5]; label_exists[ 1] = label_exists[ 5] = 1; if ( a_label < b_label ) { EQUIV_nb++; label_vertex[b_label] = a_label; for ( i=b_label+1; i<8; i++ ) if ( label_vertex[i] == b_label ) label_vertex[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_vertex[a_label] = b_label; for ( i=a_label+1; i<8; i++ ) if ( label_vertex[i] == a_label ) label_vertex[i] = b_label; } }
    ...
    if ( ( ++TESTS_nb ? neighbors[ 12] : neighbors[ 12] ) != 0 ) {
	  ...
	  { a_label = label_vertex[ 2]; b_label = label_vertex[ 6]; label_exists[ 2] = label_exists[ 6] = 1; if ( a_label < b_label ) { EQUIV_nb++; label_vertex[b_label] = a_label; for ( i=b_label+1; i<8; i++ ) if ( label_vertex[i] == b_label ) label_vertex[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_vertex[a_label] = b_label; for ( i=a_label+1; i<8; i++ ) if ( label_vertex[i] == a_label ) label_vertex[i] = b_label; } }
	...
	if ( label_vertex[3] != min_label ) { EQUIV_nb++; old_label = label_vertex[ 3 ]; label_vertex[old_label] = min_label; for ( i=old_label+1; i<8; i++ ) if ( label_vertex[i] == old_label ) label_vertex[i] = min_label; }
	if ( label_vertex[5] != min_label ) { EQUIV_nb++; old_label = label_vertex[ 5 ]; label_vertex[old_label] = min_label; for ( i=old_label+1; i<8; i++ ) if ( label_vertex[i] == old_label ) label_vertex[i] = min_label; }
	if ( label_vertex[7] != min_label ) { EQUIV_nb++; old_label = label_vertex[ 7 ]; label_vertex[old_label] = min_label; for ( i=old_label+1; i<8; i++ ) if ( label_vertex[i] == old_label ) label_vertex[i] = min_label; }
	label_exists[1] = label_exists[3] = label_exists[5] = label_exists[7] = 1;
    } else {
      if ( ( ++TESTS_nb ? neighbors[ 17] : neighbors[ 17] ) != 0 )
           { a_label = label_vertex[ 3]; b_label = label_vertex[ 7]; label_exists[ 3] = label_exists[ 7] = 1; if ( a_label < b_label ) { EQUIV_nb++; label_vertex[b_label] = a_label; for ( i=b_label+1; i<8; i++ ) if ( label_vertex[i] == b_label ) label_vertex[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_vertex[a_label] = b_label; for ( i=a_label+1; i<8; i++ ) if ( label_vertex[i] == a_label ) label_vertex[i] = b_label; } }
