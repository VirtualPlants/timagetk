  ...
  register int a_label, b_label;
	...
	if ( ( ++TESTS_nb ? neighbors[ 12] : neighbors[ 12] ) == 0 ) {
	  if ( ( ++TESTS_nb ? neighbors[ 15] : neighbors[ 15] ) == 0 )
	    { a_label = label_neighbor[ 2]; b_label = label_neighbor[ 4]; if ( a_label < b_label ) { EQUIV_nb++; label_neighbor[b_label] = a_label; for ( i=b_label+1; i<6; i++ ) if ( label_neighbor[i] == b_label ) label_neighbor[i] = a_label; } else if ( b_label < a_label ) { EQUIV_nb++; label_neighbor[a_label] = b_label; for ( i=a_label+1; i<6; i++ ) if ( label_neighbor[i] == a_label ) label_neighbor[i] = b_label; } }
