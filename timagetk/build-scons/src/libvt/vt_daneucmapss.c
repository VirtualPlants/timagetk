...
static void _VT_AutreZ( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dim[2], int dz );
static void _VT_AutreY( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dim[2], int dy );
static void _VT_AutreX( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dx );
...
int VT_DanVecteurPPP_SS( vt_image *theIm /* input image */, 
               ...
               vt_distance *par /* parameters */ )
{
    ...
    vly = (s16 *)resVy->buf;
    vlz = (s16 *)resVz->buf;
    pvlx = (s16***)resVx->array;
    pvly = (s16***)resVy->array;
    pvlz = (s16***)resVz->array;
...

{
    ...
    m = n = _VT_SS_NORME( (int)(vx[z][y][x]), (int)(vy[z][y][x]), (int)(vz[z][y][x]) );
    ddx = ddy = ddz = 0;
    zdz = z + dz;
    ...
    for ( dy = -1; dy < 2; dy ++ ) {
      ...
      for ( dx = -1; dx < 2; dx ++ ) {
          ...
          m = _VT_SS_NORME( ((int)(vx[zdz][ydy][xdx]) + dx), ((int)(vy[zdz][ydy][xdx]) + dy), ((int)(vz[zdz][ydy][xdx]) + dz) );
          if (m >= n)         continue;
    ...
    if ( ddz == dz ) {
      ...
      if ( n < _VT_SS_NINFINI )      { vx[z][y][x] = (s16)(_VT_SS_NINFINI); }
      else if ( n > _VT_SS_PINFINI ) { vx[z][y][x] = (s16)(_VT_SS_PINFINI); }
      else                           { vx[z][y][x] = (s16)(n);              }
      n = (int)(vy[zdz][ydy][xdx]) + ddy;
      if ( n < _VT_SS_NINFINI )      { vy[z][y][x] = (s16)(_VT_SS_NINFINI); }
      else if ( n > _VT_SS_PINFINI ) { vy[z][y][x] = (s16)(_VT_SS_PINFINI); }
      else                           { vy[z][y][x] = (s16)(n);              }
      n = (int)(vz[zdz][ydy][xdx]) + dz;
      if ( n < _VT_SS_NINFINI )      { vz[z][y][x] = (s16)(_VT_SS_NINFINI); }
      else if ( n > _VT_SS_PINFINI ) { vz[z][y][x] = (s16)(_VT_SS_PINFINI); }
      else                           { vz[z][y][x] = (s16)(n);              }
    }
}
...
static void _VT_AutreY( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dim[2], int dy )
{
    ...
    for ( dx = -1; dx < 2; dx ++ ) {
      ...
      m = _VT_SS_NORME( ((int)(vx[z][ydy][xdx]) + dx), ((int)(vy[z][ydy][xdx]) + dy), (int)(vz[z][ydy][xdx]) );
      if (m >= n)         continue;
...
static void _VT_AutreX( s16 ***vx, s16 ***vy, s16 ***vz, int x, int y, int z, int dx )
{
    ...
    n = _VT_SS_NORME( (int)(vx[z][y][x]), (int)(vy[z][y][x]), (int)(vz[z][y][x]) );
    xdx = x + dx;

    m = _VT_SS_NORME( ((int)(vx[z][y][xdx]) + dx), (int)(vy[z][y][xdx]), (int)(vz[z][y][xdx]) );
