...
static char *usage = "[histogram-in] [file-out]\n\
...
int main( int argc, char *argv[] )
{
  ...
  if ( VT_ReadImageAsHistogram( par.names.in, &histo ) != 1 ) {
    freeHistogram( &histo );
    VT_ErrorParse("unable to read input image/histogram\n", 0);
  }

  /* manipulations can take place here
   */
  fprintfHistogramHeaderInfo( stdout, &histo, (char*)NULL );
  (void)minMeanMaxStddev1DHistogramWithRange( &histo, par.fbound, par.lbound,
					      &(m.min), &(m.moy), &(m.max), &(m.ect) );
  fprintf( stdout, "%17.6f %17.6f", m.min, m.moy );
  fprintf( stdout, " +/- %-17.6f", m.ect );
  fprintf( stdout, " %17.6f\n", m.max );
