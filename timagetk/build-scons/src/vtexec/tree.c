...
int main( int argc, char *argv[] )
{
  ...
  if ( par.output_name[0] != '\0' && par.output_name[0] != '>' ) {
    ...
    if ( VT_AllocImage( &imres ) != 1 ) {
      VT_FreeImage( image );
      VT_Free( (void**)&image );
      API_ErrorParse_tree( _BaseName( argv[0] ), "unable to allocate output image ...\n", 0 );
    }
    ...
    if ( API_tree( image, &imres, lineoptions, (char*)NULL ) != 1 ) {
        free( lineoptions );
        VT_FreeImage( &imres );
        VT_FreeImage( image );
        VT_Free( (void**)&image );
  ...
  if ( par.vtktree_name[0] != '\0' ) {
    ...
    if ( API_symbolicTree( image, &tree, lineoptions, (char*)NULL ) != 1 ) {
        free( lineoptions );
        VT_FreeImage( image );
        VT_Free( (void**)&image );
        API_ErrorParse_tree( _BaseName( argv[0] ), "some error occurs during processing ...\n", -1 );
    ...
    if ( writeVTKLegacyFile( par.vtktree_name, &tree, (char*)NULL ) != 1 ) {
        freeTree( &tree );
        free( lineoptions );
        VT_FreeImage( image );
        VT_Free( (void**)&image );
