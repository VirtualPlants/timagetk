...
static char *usage = "[image-in] [-x] %d [-y] %d [[-z] %d]\n\
...
\n\
 $Revision: 1.1 $ $Date: 2014/11/14 13:33:51 $ $Author: gael $\n";
...
int main( int argc, char *argv[] )
{
  ...
  if ( _VT_VERBOSE_ ) {
    printf( " will get (%d,%d,%d) in ", par.x, par.y, par.z );
		      ...
		      local_par *par )
{
  ...
  while ( i < argc ) {
	...
	nb += 1;
          ...
          status = sscanf( argv[i],"%d",&(par->x) );
          if ( status <= 0 ) VT_ErrorParse( "parsing x value...\n", 0 );
          nb+=1;
      }
      else if (nb == 2) {
          status = sscanf( argv[i],"%d",&(par->y) );
          if ( status <= 0 ) VT_ErrorParse( "parsing y value...\n", 0 );
          nb+=1;
      }
      else if (nb == 3) {
          status = sscanf( argv[i],"%d",&(par->z) );
          if ( status <= 0 ) VT_ErrorParse( "parsing z value...\n", 0 );
          nb+=1;
      }
      else
