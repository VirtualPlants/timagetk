/*************************************************************************
 * morpho.c - template for executable creation
 *
 * $Id$
 *
...
int main( int argc, char *argv[] )
{
   ...
   */
  if ( argc <= 1 )
      API_ErrorParse_morpho( _BaseName( argv[0] ), (char*)NULL, 0 );
  API_ParseParam_morpho( 1, argc, argv, &par );
  ...
  if ( image == (vt_image*)NULL ) {
      API_ErrorParse_morpho( _BaseName( argv[0] ), "unable to read input image ...\n", 0 );
  }
  ...
  if ( par.element_name[0] != '\0' ) {
      if ( readStructuringElement( par.element_name, &SE ) != 1 ) {
          API_ErrorParse_morpho( _BaseName( argv[0] ), "unable to read structuring element ...\n", 0 );
      }
  }
  ...
  if ( VT_AllocImage( &imres ) != 1 ) {
      ...
      VT_FreeImage( image );
      VT_Free( (void**)&image );
      API_ErrorParse_morpho( _BaseName( argv[0] ), "unable to allocate output image ...\n", 0 );
  }
  ...
  if ( lineoptions == (char*)NULL )
      API_ErrorParse_morpho( _BaseName( argv[0] ), "unable to translate command line options ...\n", 0 );

  if ( API_morpho( image, &imres, &SE,
      ...
      VT_FreeImage( image );
      VT_Free( (void**)&image );
      API_ErrorParse_morpho( _BaseName( argv[0] ), "some error occurs during processing ...\n", 0 );
  }
  ...
  if ( par.output_type != TYPE_UNKNOWN ) {
      ...
      if ( VT_AllocImage( &imtmp ) != 1 ) {
          VT_FreeImage( &imres );
          API_ErrorParse_morpho( _BaseName( argv[0] ), "unable to allocate auxiliary image ...\n", 0 );
      }
      if ( VT_CopyImage( &imres, &imtmp ) != 1 ) {
          VT_FreeImage( &imtmp );
          VT_FreeImage( &imres );
          API_ErrorParse_morpho( _BaseName( argv[0] ), "unable to copy result image ...\n", 0 );
      }
      VT_FreeImage( &imres );
      imout = &imtmp;
    }
  ...
  if ( VT_WriteInrimage( imout ) == -1 ) {
    VT_FreeImage( imout );
    API_ErrorParse_morpho( _BaseName( argv[0] ), "unable to write output image ...\n", 0 );
  }
