/*************************************************************************
 * createTrsf.c - template for executable creation
 *
 * $Id$
 *
...
int main( int argc, char *argv[] )
{
   ...
   */
  if ( argc <= 1 )
      API_ErrorParse_createTrsf( _BaseName( argv[0] ), (char*)NULL, 0 );
  API_ParseParam_createTrsf( 1, argc, argv, &par );
  ...
  if ( lineoptions == (char*)NULL )
      API_ErrorParse_createTrsf( _BaseName( argv[0] ), "unable to translate command line options ...\n", 0 );

  if( API_INTERMEDIARY_createTrsf( par.output_name, par.template_name, par.template_fixedpoint,
                                lineoptions, (char*)NULL ) != 1 ) {
      free( lineoptions );
      API_ErrorParse_createTrsf( _BaseName( argv[0] ), "some error occurs during processing ...\n", -1 );
  }
  free( lineoptions );
