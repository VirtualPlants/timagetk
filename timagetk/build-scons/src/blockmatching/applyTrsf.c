/*************************************************************************
 * applyTrsf.c - template for executable creation
 *
 * $Id$
 *
...
int main( int argc, char *argv[] )
{
   ...
   */
  if ( argc <= 1 )
      API_ErrorParse_applyTrsf( _BaseName( argv[0] ), (char*)NULL, 0 );
  API_ParseParam_applyTrsf( 1, argc, argv, &par );
  ...
  if ( lineoptions == (char*)NULL )
      API_ErrorParse_applyTrsf( _BaseName( argv[0] ), "unable to translate command line options ...\n", 0 );



  if ( API_INTERMEDIARY_applyTrsf( par.input_name, par.output_name,
                                   par.template_name,
                                   par.input_real_transformation,
                                   par.input_voxel_transformation,
                                   par.output_real_transformation,
                                   par.output_voxel_transformation,
                                   lineoptions, (char*)NULL  ) != 1 ) {
      free( lineoptions );
      API_ErrorParse_applyTrsf( _BaseName( argv[0] ), "some error occurs during processing ...\n", 0 );
  }
  free( lineoptions );
