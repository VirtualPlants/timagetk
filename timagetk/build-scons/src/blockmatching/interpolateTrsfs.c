/*************************************************************************
 * interpolateTrsfs.c - template for executable creation
 *
 * $Id$
 *
 ...
 *
 * AUTHOR:
 * Gael Michelni (gael.michelin@inria.fr)
 *
 * CREATION DATE:
 * Mer  2 sep 2015 11:16:42 CEST
 *
 * ADDITIONS, CHANGES
 *
 */
...
int main( int argc, char *argv[] )
{
   ...
   */
  if ( argc <= 1 )
      API_ErrorParse_interpolateTrsfs( _BaseName( argv[0] ), (char*)NULL, 0 );
  API_ParseParam_interpolateTrsfs( 1, argc, argv, &par );
  ...
  if ( lineoptions == (char*)NULL )
      API_ErrorParse_interpolateTrsfs( _BaseName( argv[0] ), "unable to translate command line options ...\n", 0 );



  if( API_INTERMEDIARY_interpolateTrsfs( par.input_name, par.template_name,
                                lineoptions, (char*)NULL ) != 1 ) {
      free( lineoptions );
      API_ErrorParse_interpolateTrsfs( _BaseName( argv[0] ), "some error occurs during processing ...\n", -1 );
  }
  free( lineoptions );
